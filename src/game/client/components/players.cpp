	...
	float Intra)
{
	...
	bool Local = m_pClient->m_Snap.m_LocalClientId == ClientId;
	bool OtherTeam = m_pClient->IsOtherTeam(ClientId);
	// float Alpha = (OtherTeam || ClientId < 0) ? g_Config.m_ClShowOthersAlpha / 100.0f : 1.0f; // Alesstya1
	...
	if(!OtherTeam && g_Config.m_ClShowOthersGhosts && !Local && g_Config.m_ClUnpredOthersInFreeze && g_Config.m_ClAmIFrozen && !Spec)
		Alpha = 1.0f;
	if(ClientId == -2) // ghost
		Alpha = g_Config.m_ClRaceGhostAlpha / 100.0f; // Alesstya1
	...
	if(g_Config.m_ClSwapGhosts && g_Config.m_ClShowOthersGhosts && !Local && Client()->State() != IClient::STATE_DEMOPLAYBACK) // Alesstya1
		if(ClientId >= 0)
			Position = mix(
				...
				Client()->IntraGameTick(g_Config.m_ClDummy)); // Alesstya2
	...
	if(g_Config.m_ClAntiPingImproved && !Local) // Alesstya1
		InAir = !Collision()->CheckPoint(Position.x, Position.y + 16); // Alesstya2
	...
	vec2 GhostWeaponPos = vec2(0, 0); // Alesstya1
	// draw gun
	if(Player.m_Weapon >= 0)
	{
		if(!(RenderInfo.m_TeeRenderFlags & TEE_NO_WEAPON))
		{
			...
			GhostWeaponPos = WeaponPosition; // Alesstya1
		}
	}
...
void CPlayers::RenderPlayerGhost( // Alesstya1
	const CNetObj_Character *pPrevChar,
	const CNetObj_Character *pPlayerChar,
	const CTeeRenderInfo *pRenderInfo,
	int ClientId,
...
void CPlayers::OnRender()
{
	...
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		if(((CharacterInfo.m_Cur.m_Weapon == WEAPON_NINJA || Frozen) && g_Config.m_ClShowNinja) || (g_Config.m_ClAmIFrozen && g_Config.m_ClFreezeUpdateFix && m_pClient->m_Snap.m_LocalClientId == i && g_Config.m_ClShowNinja) || (Frozen && !m_pClient->m_GameInfo.m_NoSkinChangeForFrozen && g_Config.m_ClShowNinja)) // Alesstya1
		{
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
		...
		{
			if(!(g_Config.m_ClShowOthersGhosts && g_Config.m_ClSwapGhosts)) // Alesstya1
				continue;
		}
		...
		if(RenderGhost && g_Config.m_ClShowOthersGhosts && !Spec && Client()->State() != IClient::STATE_DEMOPLAYBACK)
			RenderPlayerGhost(&m_pClient->m_aClients[ClientId].m_RenderPrev, &m_pClient->m_aClients[ClientId].m_RenderCur, &aRenderInfo[ClientId], ClientId); // Alesstya2
